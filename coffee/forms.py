...
class CoffeeQualityForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['species'] = forms.ChoiceField(choices=[(i.species, i.species) for i in species])
        self.fields['country'] = forms.ChoiceField(choices=[(i.country, i.country) for i in countries])
        self.fields['year'] = forms.ChoiceField(choices=[(i.year, i.year) for i in years])
        self.fields['variety'] = forms.ChoiceField(choices=[(i.variety, i.variety) for i in varieties])
        self.fields['colour'] = forms.ChoiceField(choices=[(i.colour, i.colour) for i in colours])
        self.fields['processing_method'] = forms.ChoiceField(choices=[(i.processing_method, i.processing_method) for i in processing_methods])
